...
namespace {
	class Lfpa {
		...
		public :
			...
			std::set<Value *> getMayDefVariables(pointerPointeeMap, Value *, std::set<Value *> *, bool *, int *);
...
/*	std::set<Value *> getActualVariables(operand opd, Instruction *ip, bool *defnFreePath) {
	...
	*/
	...
	std::set<Value *> Lfpa::getMayDefVariables(pointerPointeeMap A_in, Value *lOp, std::set<Value *> *L_in, bool *defnFreeLhs, int *ind) {
		std::set<Value *> mayDefSet, tempSet;
		mayDefSet.insert(lOp);
		...
		for(auto bb = bbList->rbegin(); bb != bbList->rend(); bb++) {
			...
			for(auto ii = bp->rbegin(); ii != bp->rend(); ii++) {
				...
				if(isa<StoreInst>(ip)) {
					...
					int ind = lOp.second;
					Value *v = lOp.first;
					lhsSet = getMayDefVariables(A_in, lOp.first, &tL_in, &defnFreeLhs, &ind);
					if(ind == -1) {
